...
# Secret scanning. Note this costs a project nothing in its own go.mod: gitleaks
# pulls ~200 modules, but as a *tool* none of that touches a consumer's module
# graph. Importing it as a library would have been a different story.
GITLEAKS_VERSION      ?= v8.30.1

GOLANGCI_LINT := $(TOOLS_BIN)/golangci-lint-$(GOLANGCI_LINT_VERSION)
GORELEASER    := $(TOOLS_BIN)/goreleaser-$(GORELEASER_VERSION)
GITLEAKS      := $(TOOLS_BIN)/gitleaks-$(GITLEAKS_VERSION)
...
$(GITLEAKS): | $(TOOLS_BIN)
	@echo "installing gitleaks $(GITLEAKS_VERSION) (one-off, a minute or two)"
	@GOBIN=$(TOOLS_BIN) $(GO) install \
	  github.com/zricethezav/gitleaks/v8@$(GITLEAKS_VERSION)
	@mv $(TOOLS_BIN)/gitleaks $@
...
.PHONY: tools
tools: $(GOLANGCI_LINT) $(GITLEAKS) ## Install pinned developer tooling
